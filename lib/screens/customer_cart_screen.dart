...
class _CustomerCartScreenState extends State<CustomerCartScreen> {
  ...
  Future<void> _processCheckout({required double payAmount}) async {
    ...
    try {
      ...
      // 3. Compute turnaround from CartItem data
      ...
      // 4. Create PayMongo payment link for chosen amount
      ...
      // Log link → order mapping for webhook handler
      ...
      // 5. Save order to Firestore as awaiting_payment
      ...
      // 6. Update chat thread + add system message to subcollection
      ...
      final systemMsg =
          ...
          'Est. turnaround: ~$turnaroundDays day${turnaroundDays == 1 ? '' : 's'}';
  ...
  Future<void> _onPaymentConfirmed({
    ...
    // Create Order_Queue entry (FIFO by created_at)
    ...
    // Write Sales_Record for this payment
    ...
    await FirebaseFirestore.instance
        .collection('Messages')
        .doc('chat_${FirebaseAuth.instance.currentUser?.uid ?? ''}')
        .collection('chat')
        .add({
      'sender_uid':  'system',
      'sender_role': 'system',
