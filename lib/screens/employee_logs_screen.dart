...
class _AddWalkInJobDialogState extends State<_AddWalkInJobDialog> {
  ...
  // Selected product line-items: [{productDoc, qty, notes}]
  final List<_WalkInItem> _items = [];

  // Available products from Firestore
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final products = _items.map((item) => {
        'product_id':   item.productData['product_id']?.toString() ?? item.productDocId,
        'name':         item.productData['product_name']?.toString() ?? '—',
        'category':     item.productData['category']?.toString() ?? '',
        'qty':          item.qty,
        'unit_price':   (item.productData['price'] as num?)?.toDouble() ?? 0,
      ...
      batch.set(orderRef, {
        ...
        'payment_status':       fullyPaid ? 'paid' : (paidAmount > 0 ? 'partial' : 'unpaid'),
      ...
      // ── Sales Record (if payment was made) ──────────────────────────────
      if (paidAmount > 0) {
                ...
                Row(children: [
                  const Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('New Walk-In Job Order',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 17,
                                fontWeight: FontWeight.bold)),
                        SizedBox(height: 2),
                        Text('Walk-in customer · cash or GCash',
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          decoration: AppTheme.inputDecoration('Customer Name *',
                        ...
                        TextFormField(
                          ...
                          decoration: AppTheme.inputDecoration('Customer Email (optional)',
                        ...
                        Row(children: [
                          ...
                          const Spacer(),
                          GestureDetector(
                            onTap: _loadingProducts ? null : _showAddProductSheet,
                            child: Container(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                ...
                                Text(
                                  _loadingProducts ? 'Loading…' : 'Add Product',
                        ...
                        if (_items.isEmpty)
                          Container(
                            ...
                            decoration: AppTheme.glassCard(opacity: 0.08, radius: 10),
                            child: const Center(
                              ...
                              child: Row(children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        item.productData['product_name']?.toString() ?? '—',
                                        style: const TextStyle(
                                            color: Colors.white,
                                            fontWeight: FontWeight.w600,
                                            fontSize: 13),
                                      ...
                                      Text(
                                        [
                                          if (item.needsSize && item.widthFt != null && item.heightFt != null)
                        ...
                        // Payment method
                        Row(children: [
                          _payMethodChip('cash',  'Cash',  Icons.payments_outlined),
                          const SizedBox(width: 8),
                          _payMethodChip('gcash', 'GCash', Icons.phone_android_outlined),
                        ]),
                        const SizedBox(height: 10),
                        ...
                        TextFormField(
                          ...
                          decoration: AppTheme.inputDecoration('Amount Paid (₱) *',
                        ...
                        TextFormField(
                          ...
                          style: const TextStyle(color: Colors.white, fontSize: 14),
                          maxLines: 2,
                          decoration: AppTheme.inputDecoration('Order Notes (optional)',
  ...
  Widget _payMethodChip(String value, String label, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 14, color: active ? AppTheme.gold : Colors.white54),
...
class _ProductPickerDialogState extends State<_ProductPickerDialog> {
  ...
  static const _sizePresets = ['2×3 ft','3×4 ft','4×6 ft','4×8 ft','5×10 ft','Custom'];
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return GestureDetector(
                      onTap: () => setState(() {
                        ...
                        _widthCtrl.text = '2'; _heightCtrl.text = '3';
                        // Reset material to first option for new category
                        final mats = _materialMap[data['category']?.toString() ?? ''] ?? [];
              ...
              // ── Qty + notes (shown after selection) ───────────────────
              if (_selected != null) ...[
                const SizedBox(height: 12),
                const Divider(color: Colors.white12),
                const SizedBox(height: 12),

                // ── Size (only for area-priced products) ──────────────
                if (_needsSize) ...[
                  _pickerLabel('Size (in feet)'),
                  const SizedBox(height: 8),
                  Wrap(
                    spacing: 6, runSpacing: 6,
                    children: _sizePresets.map((p) => _selChip(p, _sizePreset == p, () {
                      final dims = _presetDims[p];
                      setState(() {
                        _sizePreset = p;
                        if (dims != null) {
                          _widthFt = dims.$1; _heightFt = dims.$2;
                  ...
                  Row(children: [
                    Expanded(child: _dimField('Width', _widthCtrl, (v) {
                      ...
                      if (d != null && d > 0) setState(() { _widthFt = d; _sizePreset = 'Custom'; });
                    })),
                    const Padding(
                    ...
                    Expanded(child: _dimField('Height', _heightCtrl, (v) {
                      final d = double.tryParse(v);
                      if (d != null && d > 0) setState(() { _heightFt = d; _sizePreset = 'Custom'; });
                    })),
                  ]),
                  const SizedBox(height: 14),
                ],
                ...
                if (_materialList.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 6, runSpacing: 6,
                    children: _materialList.map((m) => _selChip(m, _material == m,
                            () => setState(() => _material = m))).toList(),
                  ),
                  const SizedBox(height: 14),
                ],
                ...
                Row(children: [
                  ...
                  const Spacer(),
                  _qtyBtn(Icons.remove_rounded, () => setState(() {
                    final minQty = ((_selected!.data() as Map)['min_quantity'] as num?)?.toInt() ?? 1;
                    if (_qty > minQty) { _qty--; _qtyCtrl.text = '$_qty'; }
                  })),
                  const SizedBox(width: 8),
                  SizedBox(
                    width: 56,
                  ...
                  const SizedBox(width: 8),
                  _qtyBtn(Icons.add_rounded, () => setState(() {
                    _qty++; _qtyCtrl.text = '$_qty';
                  })),
                  const Spacer(),
                  Text('₱${_lineSubtotal.toStringAsFixed(2)}',
                      style: const TextStyle(
                          color: AppTheme.gold,
                          fontWeight: FontWeight.bold,
                          fontSize: 14)),
                ...
                child: ElevatedButton(
                  onPressed: _selected == null
                    ...
                    if (_materialList.isNotEmpty && _material == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Please select a material / finish.')),
                      );
                      return;
                    }
                    widget.onAdd(_WalkInItem(
                  ...
                  child: Text(
                    _selected == null ? 'Select a product first' : 'Add to Order',
                    style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 13,
  ...
  Widget _catChip(String label, bool active, VoidCallback onTap) => GestureDetector(
    onTap: onTap,
    child: Container(
  ...
  Widget _pickerLabel(String text) => Text(text,
      style: const TextStyle(
...
class _QueueCard extends StatelessWidget {
  ...
  Future<void> _cancelOrder(BuildContext context) async {
    ...
    // Notify customer via chat (only if not a walk-in)
    if (customerUid != null && customerUid.isNotEmpty) {
      ...
      await threadRef.collection('chat').add({
        'sender_uid':  'system',
        'sender_role': 'system',
        'text':        'Your order $orderId has been cancelled. Please contact us for assistance.',
